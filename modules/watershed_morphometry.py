...
def _mask_to_polygon(mask, transform, crs, simplify_m=80.0):
    """Vectoriza la máscara de cuenca sin depender de scikit-image.

    Usa rasterio.features.shapes para reducir dependencias y acelerar instalación
    en Streamlit Cloud.
    """
    ...
    if int(mask_bool.sum()) < 1:
        raise RuntimeError("Máscara de cuenca vacía.")
...
def delineate_basin(path_or_bytes, outlet_lon: float, outlet_lat: float, snap_radius_m: float = 1500.0, max_cells: int = 20_000_000, simplify_m: float = 80.0) -> BasinResult:
    ...
    r1, c1 = _snap(r0, c0, acc, valid, radius_cells)
